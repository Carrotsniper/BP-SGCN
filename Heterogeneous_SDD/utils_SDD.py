...
class AgentTrajectoryDataset(Dataset):
    def __init__(
        ...
        if not (np.array_equal(seq_list, [])):
            ...
            self.v_obs_ = [v.permute(1,0,2) for v in self.v_obs]
            self.v_obs_ = np.concatenate(self.v_obs_, axis=0)

            self.v_pred_ = [v.permute(1,0,2) for v in self.v_pred]
            self.v_pred_ = np.concatenate(self.v_pred_, axis=0)

            self.obs_traj = self.obs_traj.permute(0, 2, 1)
            self.pred_traj = self.pred_traj.permute(0, 2, 1)
            self.obs_traj_rel = self.obs_traj_rel.permute(0, 2, 1)
            self.pred_traj_rel = self.pred_traj_rel.permute(0, 2, 1)
    ...
    def __getitem__(self, index): # index is seq_index
        ...
        out = [
            ...
            self.non_linear_ped[index], self.loss_mask[index], 
            self.v_obs_[index],self.v_pred_[index]
        ]
        return out
